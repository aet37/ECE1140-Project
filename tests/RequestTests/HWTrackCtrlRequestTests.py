...
def test_get_hw_track_controller_request(start_server):
    """Ensures server responds with a success and the expected request on the queue

    PRECONDITIONS: A request containing (HWTRACK_GET_TAG_VALUE, "Switch1 1")
    EXECUTION STEPS: send_message(HWTRACK_GET_HW_TRACK_CONTROLLER_REQUEST)
    POSTCONDITIONS: ResponseCode is SUCCESS, data is 

    """
    response_code, data = send_message(RequestCode.HWTRACK_SEND_HW_TRACK_CONTROLLER_REQUEST,
                                       data="96 Switch1 1",
                                       ignore_exceptions=())
