...
def test_statement_tag_2(mock_emitter):
    ...
    with pytest.raises(Exception) as pytest_wrapped_e:
        par.program()
    assert "Parsing error : Expected FALSE, but found notAKeyword" == str(pytest_wrapped_e.value)
...
def test_statement_task_3(mock_emitter):
    ...
    with pytest.raises(Exception) as pytest_wrapped_e:
        par.statement()
    assert "Parsing error : Invalid task type CONTINUOUS" == str(pytest_wrapped_e.value)
