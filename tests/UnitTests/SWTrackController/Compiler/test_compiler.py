...
def test_too_many_ends():
    ...
    with pytest.raises(CompilationError) as pytest_wrapped_e:
        par.program()
    assert "Parsing error line #12 : Too many end statements" == str(pytest_wrapped_e.value)
...
def test_missing_end():
    ...
    with pytest.raises(CompilationError) as pytest_wrapped_e:
        par.program()
    assert "Parsing error line #10 : Missing matching ENDROUTINE" == str(pytest_wrapped_e.value)
...
def test_low_period():
    ...
    with pytest.raises(CompilationError) as pytest_wrapped_e:
        par.program()
    assert "Parsing error line #2 : Period below allowable limit 20" == str(pytest_wrapped_e.value)
