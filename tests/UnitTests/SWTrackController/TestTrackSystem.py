"""
	UNIT Testing for SWTrack TrackSystem File
...
import sys
sys.path.insert(1, '../../..')
from src.SWTrackController.TrackSystem import *
...
class MyTestCase(unittest.TestCase):
	...
	"""
	def test_init(self):
		self.assertEqual(len(TrackSystem.switchpos_arr) == 13, True)
		self.assertEqual(len(TrackSystem.TrackController_arr) == 26, True)
	...
	def updateOccupied(self):
		...
		self.assertEqual(len(TrackSystem.redOccupancies) == 76, True)
		self.assertEqual(len(TrackSystem.greenOccupancies) == 150, True)
		TrackSystem.updateOccupied(LINE_GREEN,50)

		self.assertEqual(TrackSystem.greenOccupancies[50] == 1, True)

		self.assertEqual(TrackSystem.blocks_green_arr[1].occupied, False)
