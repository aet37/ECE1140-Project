...
def test_get_hw_track_controller_response_failure(start_server):
    ...
    """
    response_code, data = send_message(RequestCode.GET_HW_TRACK_CONTROLLER_RESPONSE, ignore_exceptions=())
    assert(response_code == ResponseCode.ERROR)

def test_get_hw_track_controller_request(start_server):
    response_code, data = send_message(RequestCode.GET_HW_TRACK_CONTROLLER_REQUEST, ignore_exceptions=())
    assert(response_code == ResponseCode.ERROR)
