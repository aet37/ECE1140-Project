...
class SignalsClass(QObject):
    ...
    swtrack_update_occupancies = pyqtSignal(int, list, Line) # Used by Track Model to update occupancies (trainId, blockNumber, Line)
    ...
    # Train Model Signals
    train_model_dispatch_train = pyqtSignal(int, int, float, int, int, list) #  Used by the track model to signify that a new train has been dispatched FORMAT: (train_id, destination_block, command_speed, authority, Line, route)
