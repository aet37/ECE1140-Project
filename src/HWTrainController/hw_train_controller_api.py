"""API for the HW TRAIN Controller Module."""
...
HOST = '3.23.104.34'
SERVER_PORT = 1234
GET_HW_TRAIN_CONTROLLER_REQUEST = b'224'
SEND_HW_TRAIN_CONTROLLER_RESPONSE = b'225' 
...
def get_request():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        sock.sendall(GET_HW_TRAIN_CONTROLLER_REQUEST)
        data = sock.recv(1024)
...
def send_reponse_to_server(response):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.connect((HOST, SERVER_PORT))
        sock.sendall(SEND_HW_TRAIN_CONTROLLER_RESPONSE + response[1:])
        data = sock.recv(1024)
...
def main():
    ...
    argument_parser = ArgumentParser(
        prog='python hw_train_controller_api.py',
        description='Polls the server to check for request from '
                    'the hw train controller. Fulfills requests if any are found'
	)
